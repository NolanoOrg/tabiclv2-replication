#!/bin/bash
# Watchdog for the Stage-2 trainer: restart-with-resume + health auto-halt.
# Halts if: loss ema > 0.9 past step 2000, multiplier > 30 with ema > 0.65
# (stage-1 converged mult 24.9 is the healthy baseline), or 'loss nan'.
# Usage: nohup bash watchdog_stage2.sh &
...
STEPS=40000
CKPT=$RUN_DIR/ckpt_stage2
GEN=10.0.0.7:29800
cd "$REPO_DIR"
[ -f "$REPO_DIR/.venv/bin/activate" ] && source "$REPO_DIR/.venv/bin/activate"
LOG=$RUN_DIR/logs/run_stage2_full.log
HALT=$RUN_DIR/HALT2
WLOG=/tmp/watchdog_stage2.log
...
health_check() {
  ...
  if [ -n "$EMA" ] && [ "$EMA" != "nan" ] && [ "${STEP:-0}" -gt 2000 ]; then
...
while true; do
  if [ -f "$HALT" ]; then
    echo "[watchdog $(date -u +%FT%T)] HALT2 present — not restarting" >> $WLOG
    sleep 300; continue
  fi
  if grep -q "^Done" "$LOG" 2>/dev/null; then
    echo "[watchdog $(date -u +%FT%T)] stage 2 complete — exiting" >> $WLOG
    exit 0
  fi
  REASON=$(health_check) || {
    echo "[watchdog $(date -u +%FT%T)] HEALTH HALT: $REASON" >> $WLOG
    echo "$(date -u +%FT%T) $REASON" > "$HALT"
    pgrep -f "python -u pretraining/stage2_[t]rain.py" | xargs -r kill -9
    continue
  }
  if ! pgrep -f "python -u pretraining/stage2_[t]rain.py" >/dev/null; then
    echo "[watchdog $(date -u +%FT%T)] trainer down — (re)starting with --resume auto" >> $WLOG
    PYTHONHASHSEED=0 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True nohup python -u pretraining/stage2_train.py \
      --steps $STEPS --remote_gen $GEN --checkpoint_dir $CKPT \
      --save_every 1000 --log_every 20 --resume auto >> "$LOG" 2>&1 &
  fi
  sleep 60
done
